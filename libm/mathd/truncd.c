...
double trunc(double x)
{
    ...
    exponent_less_1023 = (int32_t)((msw & 0x7ff00000U) >> 20) - 1023;

    if (exponent_less_1023 < 20) {
        /* All significant digits are in msw. */
        if (exponent_less_1023 < 0) {
            /* -1 < x < 1, so result is +0 or -0. */
            INSERT_WORDS(x, sb, 0U);
        } else {
            /* All relevant fraction bits are in msw, so lsw of the result is 0. */
          INSERT_WORDS(x, sb | (msw & ~(0x000fffffU >> (uint32_t)exponent_less_1023)), 0U);
        }
      ...
      INSERT_WORDS(x, msw, lsw & ~(0xffffffffU >> (uint32_t)(exponent_less_1023 - 20)));
    }
